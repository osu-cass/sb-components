...
> {
  ...
  renderResourceType(type: string) {
    ...
    const resourceTypeHeader = <h3>{type}</h3>;
    ...
    const dropdowns = resources.map(res => {
      ...
      const ddprops: Dropdown.DropdownProps = {
    ...
    if (resCount <= 4) {
      ...
      const ariaText = "Display fewer " + type + "options";
      expandButton = (
        <a
      ...
      const ariaText = "Display all " + type + " options";
      expandButton = (
        <a
          className="expand-button"
          tabIndex={0}
  ...
  render() {
    ...
    return (
      <div
        ...
        tabIndex={-1}
        role="dialog"
        aria-labelledby="Accessibility Options Modal"
        aria-describedby="Accessibility Options Modal"
        aria-hidden="true"
      >
