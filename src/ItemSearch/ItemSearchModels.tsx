...
export function filterItemCards(
    ...
    filter: SearchAPIParamsModel) : ItemCardModel[] {

    let results = itemCards;
    //item
    if(filter.itemId && filter.itemId !== ""){
        results = results
            .filter(i => i.itemKey.toString().includes(filter.itemId || ""));
    }
    ...
    if(filter.subjects && filter.subjects.length > 0){
        results = results.filter(i => (filter.subjects || [])
            .findIndex(s => s === i.subjectCode));
    }

    //interaction types
    if(filter.interactionTypes && filter.interactionTypes.length > 0){
        results = results.filter(i => (filter.interactionTypes || [])
            .findIndex(it => it === i.interactionTypeCode ));
    }

    //claims
    if(filter.claims && filter.claims.length > 0){
        results = results.filter(i => (filter.claims || [])
            .findIndex(c => c === i.claimCode));
    }
    ...
    if(filter.targets && filter.targets.length > 0){
        results = results.filter(i => (filter.targets || [])
            .findIndex(t => t === i.targetHash)) 
    }
